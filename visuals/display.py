...
class PPM:
    ...
    def paint(self, file_path: str, fmt: str) -> None:
        """
...
def load_images_starting_with(
    root: str, prefix: str, title: bool = False
) -> List[Image]:
    imgs = []
    index = 0
...
def generic_out(data, rules: Dict[str, str], prefix, index):
    multiplier = 30
    final = []
    for d in data:
        d = [d] * multiplier
...
def generate_gif(root, prefix):
    imgs = load_images_starting_with(root, prefix, title=True)
    imgs[0].save(
        rf"./images/{prefix}.gif",
        save_all=True,
